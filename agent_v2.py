...
@tool
def load_credentials(n: int = 5) -> str:
    """Load the next N credentials from the wordlist that have NOT been tried yet."""
    ...
    # only return creds not already tried
    fresh = [c for c in lines if c not in USED_CREDENTIALS]
    creds = fresh[:n]
    out = "\n".join(creds) if creds else "No new credentials available"
...
def ssh_attempt(ip: str, port: int, username: str, password: str) -> str:
    """Attempt SSH login. Returns success, auth_failed, or connection_refused."""
    cred = f"{username}:{password}"
    USED_CREDENTIALS.add(cred)  # track it
...
def generate_session_report(findings: str) -> str:
    ...
    report_path = os.path.join(LOG_DIR, f"report_{SESSION_ID}_v2.txt")
...
generate_session_report.name = "generate_session_report"
TOOL_MAP = {t.name: t for t in TOOLS}
TOOL_MAP["submit_session_report"] = generate_session_report
...
def tool_node(state: AgentState):
    ...
    for call in last.tool_calls:
        ...
        if fn is None:
            print(f"[!] Unknown tool: {call['name']} — skipping")
            continue
...
SYSTEM_PROMPT = (
    ...
    "Follow this exact methodology every time:\n"
    "1. ping_host first — confirm target is alive\n"
    "2. nmap_scan — identify all open ports and service versions\n"
    "3. banner_grab — grab banner on every open port\n"
    "4. http_probe — always probe HTTP regardless of what nmap shows\n"
    "5. load_credentials — load 5 credentials at a time\n"
    "6. ssh_attempt — try ALL 5 before loading more\n"
    "7. CRITICAL: Only call load_credentials again after attempting every credential from the previous batch\n"
    "8. After 3 consecutive auth_failed, load a new batch and continue\n"
    "9. Only call generate_session_report after trying ALL credentials\n"
...
def run_simulation(sim_name: str, user_prompt: str):
    global SESSION_ID, AGENT_LOG, USED_CREDENTIALS
    USED_CREDENTIALS = set()  # reset for each sim
